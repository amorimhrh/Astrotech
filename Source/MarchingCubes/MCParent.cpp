	...
	if((CameraLastChunkPosition != CameraChunkPosition || bArraysChanged || bFirstFrame) && ((!bAlreadyCreated && bCreateOnlyOne) || !bCreateOnlyOne))
	{
		...
		while(!bFinishedCircling)
		{
			...
			if(!(bLimitRenderHeight && (ZValue < RenderHeightMin || ZValue > RenderHeightMax)) && !IgnorePositions.Contains(ChunkCoord))
			{
				...
				if(!OccupiedPositions.Contains(NewChunkPos) && !Chunks.Contains(NewChunkPos))
				{
				...
				else if(ChunkResolutions.Contains(NewChunkPos) && Chunks.Contains(NewChunkPos))
				{
			...
			else if (IgnorePositions.Contains(ChunkCoord) && Chunks.Contains(NewChunkPos))
			{
				Chunks[NewChunkPos]->Destroy();
			}
...
void AMCParent::SpawnMesh(const FVector& Location, const int32& MicroResolution)
{
	...
	
	{
		...
		CubeMesh->NumChunksFinished = &NumChunksFinished;
...
void AMCParent::ChunkProgressInfo(int32& OutCreatedChunks, int32& OutFinishedChunks)
{
	OutCreatedChunks = NumChunksCreated - IgnorePositions.Num();
	OutFinishedChunks = NumChunksFinished;
}
